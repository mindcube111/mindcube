# 🚀 部署命令使用说明

## 📋 快速开始

### Windows 系统（推荐）

#### 方式一：双击运行（最简单）
```
双击运行 deploy.bat
```

#### 方式二：PowerShell 脚本
```powershell
.\scripts\deploy.ps1
```

或使用 npm 命令：
```powershell
npm run deploy:ps1
```

#### 方式三：Node.js 脚本（跨平台）
```bash
npm run deploy
```

### Linux/Mac 系统

```bash
npm run deploy
```

## 📦 可用的部署命令

### 1. 完整部署流程（推荐）

**Windows PowerShell:**
```powershell
.\scripts\deploy.ps1
```

**Node.js（跨平台）:**
```bash
npm run deploy
```

**功能：**
- ✅ 检查 Node.js 环境
- ✅ 检查并安装依赖
- ✅ 清理旧构建
- ✅ 构建项目
- ✅ 验证构建产物
- ✅ 检查 Wrangler CLI
- ✅ 提供部署选项（自动/手动）

### 2. 仅构建项目

```bash
npm run deploy:build
```

或

```bash
npm run build
```

**功能：**
- ✅ 构建项目到 `dist` 目录
- ✅ 复制 Functions 文件

### 3. 快速部署（需要已安装 Wrangler）

```bash
npm run deploy:quick
```

**功能：**
- ✅ 构建项目
- ✅ 直接部署到 Cloudflare Pages

**前提条件：**
- 已安装 Wrangler CLI
- 已登录 Cloudflare

### 4. 使用 Wrangler CLI 手动部署

```bash
# 1. 构建项目
npm run build

# 2. 部署
wrangler pages deploy dist --project-name=psychological-assessment-platform
```

## 🎯 部署流程说明

### 步骤 1: 环境检查
- 检查 Node.js 版本（推荐 16+）
- 检查依赖是否安装

### 步骤 2: 依赖安装
- 如果 `node_modules` 不存在，自动安装依赖

### 步骤 3: 清理构建
- 删除旧的 `dist` 目录

### 步骤 4: 构建项目
- 运行 `vite build`
- 复制 Functions 文件到 `dist/functions`

### 步骤 5: 验证构建产物
- 检查必需文件是否存在：
  - `dist/index.html`
  - `dist/functions/_middleware.js`
  - `dist/functions/api/[[path]].js`
  - `dist/assets/` 目录

### 步骤 6: 检查 Wrangler
- 检查 Wrangler CLI 是否安装
- 如果未安装，可选择自动安装

### 步骤 7: 部署选项
提供三种部署方式：

1. **Wrangler CLI 自动部署**（推荐）
   - 自动登录（如需要）
   - 直接部署到 Cloudflare Pages
   - 显示部署后的网站地址

2. **Cloudflare Dashboard 手动部署**
   - 显示手动部署步骤
   - 提供 `dist` 目录位置

3. **仅构建，稍后部署**
   - 仅完成构建，不部署
   - 稍后手动部署

## 🔧 部署前准备

### 必需工具

1. **Node.js 16+**
   - 下载地址: https://nodejs.org/
   - 检查版本: `node --version`

2. **npm**
   - 通常随 Node.js 一起安装
   - 检查版本: `npm --version`

3. **Wrangler CLI**（可选，用于 CLI 部署）
   - 安装: `npm install -g wrangler`
   - 登录: `wrangler login`

### 环境变量（如需要）

如果项目需要环境变量，请在 Cloudflare Dashboard 中配置：
1. 进入 Pages 项目设置
2. 选择 "Environment variables"
3. 添加所需的变量

## 📝 部署后验证

部署完成后，访问以下地址验证：

- **网站地址**: `https://psychological-assessment-platform.pages.dev`
- **API 测试**: `https://psychological-assessment-platform.pages.dev/api/questionnaires/available`

## 🛠️ 常见问题

### Q1: 构建失败怎么办？

**解决方案：**
1. 确保所有依赖已安装: `npm install`
2. 检查 Node.js 版本（推荐 18+）
3. 查看构建错误信息并修复
4. 清理后重新构建: `rm -rf dist node_modules && npm install && npm run build`

### Q2: Wrangler 未安装怎么办？

**解决方案：**
1. 运行部署脚本时会提示安装
2. 或手动安装: `npm install -g wrangler`
3. 或使用 Cloudflare Dashboard 手动部署

### Q3: Functions 不工作？

**解决方案：**
1. 确认 `dist/functions` 目录存在
2. 检查构建脚本是否包含 `copy-functions`
3. 在 Cloudflare Dashboard 中查看 Functions 状态
4. 检查 `dist/functions/api/[[path]].js` 是否存在

### Q4: 部署后网站显示 404？

**解决方案：**
1. 等待 2-5 分钟让域名生效
2. 清除浏览器缓存
3. 检查 Cloudflare Dashboard 中的部署状态
4. 查看构建日志中的错误信息

### Q5: API 路由返回 404？

**解决方案：**
1. 确认 `dist/functions/api/[[path]].js` 存在
2. 检查 API 路径是否正确（应该是 `/api/...`）
3. 查看 Cloudflare Workers 日志
4. 确认 Functions 已正确部署

## 🔄 更新部署

每次代码更新后，重新运行部署命令：

```bash
# 方式一：使用完整部署脚本（推荐）
npm run deploy

# 方式二：快速部署（需要已安装 Wrangler）
npm run deploy:quick

# 方式三：手动部署
npm run build
wrangler pages deploy dist --project-name=psychological-assessment-platform
```

## 📚 相关文档

- [完整部署指南](部署指南.md)
- [快速部署指南](快速部署.md)
- [Cloudflare Pages 文档](https://developers.cloudflare.com/pages/)
- [Cloudflare Workers 文档](https://developers.cloudflare.com/workers/)

## ✅ 部署检查清单

部署前请确认：

- [ ] Node.js 16+ 已安装
- [ ] 依赖已安装 (`npm install`)
- [ ] 项目可以本地构建 (`npm run build`)
- [ ] `dist/functions` 目录存在且有内容
- [ ] 所有环境变量已配置（如需要）
- [ ] Wrangler CLI 已安装（如使用 CLI 部署）
- [ ] 已登录 Cloudflare（如使用 CLI 部署）

---

**提示：** 首次部署建议使用完整部署脚本 (`npm run deploy`)，它会自动检查所有前置条件并引导您完成部署。










